...
# -------------------------------------------------------------------------------------
def _print_qname(qname, module: "Module") -> str:
    module_str = (
        f"{c2str(qname.mod.mod.name)}:"
        if qname.mod not in (ffi.NULL, module.cdata.parsed)
        else None
    )
    ret = module_str if module_str is not None else ""
    ret += c2str(qname.str)
    return ret
...
class PType:
    ...
    def range(self) -> Optional[str]:
        if self.cdata.range == ffi.NULL:
            return None
        return _print_qname(self.cdata.range.arg, self.module)

    def length(self) -> Optional[str]:
        if self.cdata.length == ffi.NULL:
            return None
        return _print_qname(self.cdata.length.arg, self.module)
...
class PRefine:
    ...
    def defaults(self) -> Iterator[str]:
        for d in ly_array_iter(self.cdata.dflts):
            yield _print_qname(d, self.module)
...
class PNode:
    ...
    def __init__(self, context: "libyang.Context", cdata, module: Module) -> None:
        self.context = context
        self.cdata = cdata  # C type of "struct lysp_node *"
        self.module = module
...
class PLeaf(PNode):
    ...
    def default(self) -> Optional[str]:
        return _print_qname(self.cdata_leaf.dflt, self.module)
...
class PList(PNode):
    ...
    def uniques(self) -> Iterator[str]:
        for u in ly_array_iter(self.cdata_list.uniques):
            yield _print_qname(u, self.module)
...
class PChoice(PNode):
    ...
    def default(self) -> Optional[str]:
        return _print_qname(self.cdata_choice.dflt, self.module)
